...
List<QuizItem> get category2_3 {
  final quizItems = [
    QuizItem(
      question: 'ソフトウェア開発のパラダイムで、データとそのデータに関連する操作をひとまとめにした「オブジェクト」を基本的な構成要素とする手法は何か？',
      ans: 'オブジェクト指向',
      choices: ["オブジェクト指向", "プロセス中心アプローチ", "データ中心アプローチ", "UML"],
      comment: "オブジェクト指向は、データ（属性）とそのデータに対する操作（メソッド）をひとまとめにしたオブジェクトを基本的な構成要素とするソフトウェア開発の手法。",
    ),
    QuizItem(
    ...
    QuizItem(
      ...
      comment: "UML（Unified Modeling Language）は、ソフトウェアの構造や振る舞い、およびビジネスプロセスを視覚的に表現するための標準的なモデリング言語。",
    ),
    QuizItem(
      question: '開発（Development）と運用（Operations）の連携を重視し、ソフトウェアのリリースや運用を迅速かつ効率的に行う文化や実践を何というか？',
      ans: 'DevOps',
      choices: ["DevOps", "アジャイル", "スクラム", "XP"],
      comment: "DevOpsは、ソフトウェア開発（Development）とIT運用（Operations）の協力を促進し、継続的なインテグレーション、デリバリー、デプロイメントを実現する文化や実践のこと。",
    ),
    QuizItem(
      question: 'ソフトウェア開発プロジェクトを、要件定義から設計、実装、テスト、リリースまでの一連のフェーズに沿って進めるモデルを何というか？',
      ans: 'ウォータフォールモデル',
    ...
    QuizItem(
      ...
      comment: "スパイラルモデルは、プロジェクトの各段階でリスク分析を行いながら、計画、リスク分析、エンジニアリング、評価のフェーズを反復して進める開発モデル。",
    ),
    QuizItem(
      question: '初期段階で実用的なモデルを迅速に開発し、ユーザーフィードバックを基に改良を加えていく開発モデルを何というか？',
      ans: 'プロトタイピングモデル',
      choices: ["プロトタイピングモデル", "ウォータフォールモデル", "スパイラルモデル", "RAD"],
      comment: "プロトタイピングモデルは、初期のプロトタイプ（試作品）を作成し、ユーザーのフィードバックを得ながら繰り返し改良を加えていくソフトウェア開発モデル。",
    ),
    QuizItem(
    ...
    QuizItem(
      ...
      comment: "XP（エクストリーム・プログラミング）は、継続的な顧客のフィードバックと小さなリリースを重視し、コーディング規約の遵守、ペアプログラミング、テスト駆動開発などを取り入れるアジャイル開発の手法。",
    ),
    QuizItem(
      question: '開発初期にテストケースを作成し、そのテストが通過するようにコードを書く開発手法を何というか？',
      ans: 'テスト駆動開発',
      choices: ["テスト駆動開発", "リファクタリング", "ペアプログラミング", "スクラム"],
      comment: "テスト駆動開発（TDD: Test-Driven Development）は、テストを先に書き、そのテストをパスする最小限のコードを実装し、次にそのコードをリファクタリングするという短い開発サイクルを繰り返す手法。",
    ),
    QuizItem(
    ...
    QuizItem(
      ...
      comment: "CMMI（Capability Maturity Model Integration）は、組織のプロセスの成熟度を段階的に評価し、ソフトウェア開発やプロジェクト管理のプロセス改善を支援するためのモデル。",
    ),

  ];

  return List.generate(quizItems.length, (index) {
    return quizItems[index].copyWith(quizId: index + 21 + 2000);
  });
}
